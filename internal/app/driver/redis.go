...
func NewCache(option CacheOption) *redis.Pool {
	...
	pool := &redis.Pool{
		Dial: func() (redis.Conn, error) {
			redisAddr := fmt.Sprintf("redis://%s@%s:%d", option.Password, option.Host, option.Port)
			c, err := redis.DialURL(redisAddr, dialConnectTimeoutOption, readTimeoutOption, writeTimeoutOption)
			if err != nil {
